...
- 文字サイズは 7 段階（11 / 12 / 13 / 14 / 15 / 20 / 26px）。段階の外は、プロフィールのパネルの写真の頭文字（`text-avatar`、112px）と、LP の最初の見出し（`text-display` 52px / `text-display-sm` 32px）だけ。`text-base` は 14px（UI 部品）、メッセージ本文と入力欄は `text-lg`（15px）。行送りは本文が `leading-relaxed`（1.75）。
