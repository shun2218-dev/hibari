...
func insertMessage(t *testing.T, tx pgx.Tx, roomID, sender ulid.ULID, seq int64, replyTo *ulid.ULID) ulid.ULID {
	t.Helper()
	mid := ids.New()
	mustExec(t, tx, `INSERT INTO messages (id, room_id, seq, sender_id, client_msg_id, body, reply_to_id, created_at)
		VALUES ($1, $2, $3, $4, $5, 'hi', $6, $7)`, mid, roomID, seq, sender, ids.New(), replyTo, now)
	return mid
}
...
func TestMessagesConstraints(t *testing.T) {
	...
	inTx(t, pool, func(tx pgx.Tx) {
		...
		const insert = `INSERT INTO messages (id, room_id, seq, sender_id, client_msg_id, body, reply_to_id, created_at)
			VALUES ($1, $2, $3, $4, $5, 'hi', $6, $7)`
...
func allocateAndInsert(ctx context.Context, pool *pgxpool.Pool, roomID, sender ulid.ULID, commit bool) (int64, error) {
	...
	seq, err := store.New(tx).AllocateMessageSeq(ctx, store.AllocateMessageSeqParams{RoomID: roomID, Now: now})
	if err != nil {
		return 0, fmt.Errorf("allocate: %w", err)
	}
	if _, err := tx.Exec(ctx, `INSERT INTO messages (id, room_id, seq, sender_id, client_msg_id, body, created_at)
		VALUES ($1, $2, $3, $4, $5, 'hi', $6)`, ids.New(), roomID, seq, sender, ids.New(), now); err != nil {
