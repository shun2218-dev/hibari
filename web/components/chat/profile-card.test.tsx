...
type MemberCard = Extract<ProfileCardView, { kind: "member" }>;
...
describe("ProfileCard（ADR 0050）", () => {
  ...
  it("オフラインは、ドットも文言も出さない（最終オンライン時刻も出さない。ADR 0049）", () => {
    render(<ProfileCard profile={member({ presence: "offline" })} />);
  ...
  it("「DM を送る」は応答を待つ間、押せない（決定 4）", () => {
    render(<ProfileCard profile={member()} dmPending />);

    expect(screen.getByRole("button", { name: "DM を送る" })).toBeDisabled();
  });

  it("email の応答待ちは、読み込み中として行だけ出す", () => {
    render(<ProfileCard profile={member({ email: { state: "loading" } })} />);

    expect(screen.getByRole("status", { name: "メールアドレスを読み込み中" })).toBeInTheDocument();
  });

  it("email が未検証なら、行もメニューのコピーも出さない（決定 2）", () => {
    render(<ProfileCard profile={member({ email: { state: "none" } })} menuOpen />);
  ...
  it("操作できない相手のメニューは、コピーの 2 つだけ", () => {
    render(<ProfileCard profile={member()} menuOpen />);
  ...
  it("自分のカードは「DM を送る」の代わりに「プロフィールを編集」", async () => {
    ...
    render(<ProfileCard profile={member({ isSelf: true })} onEditProfile={onEditProfile} />);
  ...
  it("外された人は、名前と handle だけの縮めたカードにする（決定 5）", async () => {
    ...
    render(<ProfileCard profile={former} onCopyHandle={onCopyHandle} />);
