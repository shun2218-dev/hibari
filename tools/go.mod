// 開発ツールのバージョンを固定するための専用モジュール。
// アプリ本体の go.mod に混ぜないのは、sqlc / golangci-lint の巨大な依存ツリーを
// アプリの依存グラフ（と脆弱性スキャンの対象）に持ち込まないため。
// 呼び出しは `go tool -modfile=tools/go.mod <name>`。
module github.com/shun2218-dev/hibari/tools
