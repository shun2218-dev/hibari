...
- **パスワード再設定の失敗の表示**（Phase 6-2 で判明）: `auth/forgot.png` で回数制限（429）になったとき、`auth/reset.png` で新しいパスワードが制約を満たさない（422。8 文字未満など）ときの表示がない。いまはフォームを戻すだけにしている（強度の表示は「短すぎます」になる）。再設定のトークンは 422 では消費されないので、入力し直せば同じリンクで続けられる。
- **確認リンクを開いたあとの待ち時間と通信の失敗**（Phase 6-2 で判明）: `/verify-email` は開くとすぐ確認の API を呼ぶが、結果を待つ間と、サーバーに届かないときの画面がない。いまは何も描かない。
- **ワークスペースが 0 件のときの画面**（Phase 6-2 で判明）: 招待なしで登録した直後のユーザーが最初に見る画面がない。`chat/` の画面はワークスペースがある前提。本番のページの構築順 2 の前に足す。
