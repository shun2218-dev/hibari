...
ADR 0066 のとおり足した。新しいトークンは足していない。Slack のハドルの画面の実物は見ていない（公式のヘルプとオーナーの回答から描いた）。

| 画面 | スクリーンショット | 関連する API・イベント |
|---|---|---|
| チャンネルで進行中（入っていない）: ヘッダーの「参加」・サイドバーの印・会話のメッセージ | `chat/huddle/active.png` | ルームの応答の `huddle`、`huddle.updated` |
| 入っている: サイドバーの下の窓（話している人・ミュートしている人）・ヘッダーの「退出」 | `chat/huddle/joined.png`、`chat/huddle/joined-dark.png`、`chat/huddle/mobile-joined.png`、`chat/huddle/mobile-joined-list.png` | `POST /rooms/{id}/huddle/participants`、`DELETE …/participants/{id}` |
| 自分がミュートしている | `chat/huddle/muted.png` | `PATCH …/participants/{id}` |
| 接続している / 再接続している | `chat/huddle/connecting.png`、`chat/huddle/reconnecting.png` | — |
| 終わったハドルのメッセージ（所要時間と参加した人） | `chat/huddle/ended.png` | `message.updated` |
| DM の呼び出し | `chat/huddle/dm-ring.png`、`chat/huddle/mobile-dm-ring.png` | `huddle.ringing` |
| DM で相手が「もうすぐ参加する」を押した | `chat/huddle/dm-joining-soon.png` | `POST /huddles/{id}/joining-soon` |
| DM の不在着信と応答なし | `chat/huddle/dm-missed.png` | — |
| 入れなかった・外れた（マイクを使えない / 人数の上限 / 切断された） | `chat/huddle/mic-denied.png`、`chat/huddle/full.png`、`chat/huddle/disconnected.png` | 409 `huddle-full`、心拍の ack の `not_found` |

- **ヘッダーのボタン**は、始める・入る・抜けるを 1 つで切り替える（Slack の ⌘⇧H と同じ）。進行中でなければヘッドフォンのアイコンだけ、進行中は入っている人のアバターと「参加」（押せるので緑）、入っていれば「退出」。
  モバイルは名前の幅が足りないのでアイコンだけにし、入っている間は上の帯に「退出」があるのでヘッダーには出さない。
  入れない人（参加していない public ルーム・アーカイブ済み）には出さない（ADR 0066 決定 7）。**ヘッダーが写る画面は、すべてこのアイコンの入った形に撮り直した。**
- **サイドバーの印**は名前の横の琥珀のヘッドフォン。「いま起きていること」なので琥珀にし、押せるようにはしない（入るのはヘッダーか会話のメッセージから）。
- **ハドルの窓**は md 以上でサイドバーの下（Slack の「サイドバーのハドルウィンドウ」）。ルーム名・人数・参加者のアバター・「もうすぐ参加します」・マイクと「退出」。
  - 話している人はアバターの周りの琥珀の輪、ミュートしている人は右下にマイクの斜線。
  - マイクのボタンはアイコンだけ（Slack の窓と同じ）。ミュート中は押せば戻せることを緑の地で見せる。
  - モバイルは、一覧とルームのどちらを見ていても見えるよう、検索の帯の下に 1 行の帯で出す。
- **会話のメッセージ**は、ほかのシステムメッセージ（中央寄せの 1 行）と違い、始めた人のアバターと名前の行にしてカードを置く（参加のボタンと参加者を持つため）。
  進行中はアイコンの地を琥珀にし、入っている人の顔と名前、「参加」（自分が入っていれば「参加中」）。終わったら所要時間と「あなた、佐藤 直樹、ほか 3 人が参加しました」。
  DM で入らなかった人には「不在着信」、始めた人には「応答なし」（同じメッセージの見え方の違い。ADR 0066 決定 12）。
- **DM の呼び出し**は、md 以上で右下、モバイルで上に重ねる。操作を奪わないようダイアログにはしない。ボタンは「もうすぐ参加する」と「参加」だけ（拒否はない）。
- **入れなかった・外れたとき**は、窓と同じ場所に理由を出す。入り直せるものには「もう一度参加」を付ける。マイクが見つからない・つながらなかった・権限が変わって外れた、も同じ形（story だけで撮っていない）。

#### デザインで決めた点（オーナーに確認する）

- 会話のハドルのメッセージを、始めた人の発言の形にした（ほかのシステムメッセージと形が違う）。
- 呼び出しの置き場所（md 以上は右下、モバイルは上）。Slack の実物の置き場所は確かめていない。
- マイクのボタンをアイコンだけにした。窓のボタンは「マイク」と「退出」だけ（カメラと画面共有は 6.18b）。
