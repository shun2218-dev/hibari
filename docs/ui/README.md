...
### Phase 6.9 で足した画面（プロフィールのカード）

ADR 0050 のとおり、既存の部品とトークンのまま足した（新しいトークンは要らなかった）。

| 画面 | スクリーンショット | 関連する API |
|---|---|---|
| 他人のカード | `chat/profile/other.png` | メンバー一覧の 1 行 + `GET /workspaces/{id}/members/{userID}` の `email` |
| 他人のカード（3 点メニュー） | `chat/profile/other-menu.png` | — |
| 同（ダーク） | `chat/profile/other-menu-dark.png` | — |
| 操作できる相手のメニュー（ロールの変更と削除） | `chat/profile/manage-menu.png` | `PATCH` / `DELETE /workspaces/{id}/members/{userID}` |
| 自分のカード | `chat/profile/self.png` | — |
| email の読み込み中 | `chat/profile/email-loading.png` | `GET /workspaces/{id}/members/{userID}` |
| email が未検証 | `chat/profile/email-unverified.png` | 同上（`email: null`） |
| 外された人のカード | `chat/profile/former.png` | —（メッセージの送信者の値だけ） |
| メンバーパネルから開いたところ | `chat/profile/from-members.png` | — |
| 他人のカード（モバイル） | `chat/profile/mobile-card.png` | — |

- **開く場所**: メッセージの送信者のアバターと名前（チャンネルとスレッド）、メンバーパネルの行。
  名前は読み上げではアバターのボタンと同じ操作になるので、Tab ではアバターだけに止める。
- **デスクトップは押したアバターの横に浮かせる**（行には重ねない。右に入らなければ左。メンバーパネルからはパネルの左に出る）。
  置き方は `lib/anchored-position.ts` の `placeBeside`。**モバイルは下から出るシート**（リアクションのピッカーと同じ）。
- **カードの並び**: アバター（56px。presence のドット付き）・表示名とステータスの絵文字・`@handle`・ロールのバッジと presence の文言、
  その下にステータスの文言と期限、email、操作の行。オフラインは文言も出さない（ドットと同じ扱い。最終オンライン時刻は出さない）。
- **主ボタンは「DM を送る」、自分なら「プロフィールを編集」**。横の「…」にコピーの 2 つと、操作できる相手のときだけロールの変更と削除。
  ロールの並びと削除の見た目は管理画面のメニュー（`workspace/member-list`）と同じにした。
- **email は応答を待つ間、行の高さを取って薄い帯を出す**。未検証なら行もコピーも出さない（ADR 0050 決定 2）。

#### デザインで決めた点（オーナーに確認する）

- **キックの文言は「ワークスペースから削除」**にした。ロードマップの確定事項では「ワークスペースから外す」と書いたが、
  管理画面の同じ操作が「ワークスペースから削除」で、確認のダイアログ（`KickMemberDialog`）も共通なので、そろえた。
- **外された人のカードは、メニューを出さずに「ハンドルをコピー」のボタンを 1 つだけ**置いた（ADR 0050 決定 5 では
  「メニューはハンドルのコピーだけ」。操作が 1 つなら、メニューに隠すより直接押せる方がよい）。
- **presence の文言（「オンライン」「離席中」）をロールの横に出した**。ドットだけだと、カードの大きいアバターでも色の違いが小さい。
  管理画面のメンバー一覧も文言を出している。
- **3 点メニューは上に開く**。カードは画面の下に寄せられることがあり、下に開くと画面からはみ出す。
