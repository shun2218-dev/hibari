...
- 実装で決めたこと（ADR 0037）: スレッドは `?thread=` の URL で開く。検索している間はサイドバーの「スレッド」を出さない。
...
ADR 0042 のメンションを、既存の部品とトークンのまま足した（新しいトークンは要らなかった）。
...
- **本文のチップ**: 個人は `@表示名` で、`--color-primary-subtle` の上に `--color-primary`。Phase 6.9 でプロフィールのカードを開くので押せる。
  `@channel` / `@here` は押せないので `--color-surface-muted` の上に `--color-text-secondary`。
  「緑 = 操作できるもの、琥珀 = いま起きていること」（`tokens.md`）をそのまま当てた。
...
- **サイドバーのバッジ**: メンションがあるときだけ中身を `@2` にする。色と形は未読のバッジのまま。
  未読の数はそのとき出ない（バッジは 1 行に 1 つ）。未読を太字で補うかはオーナーに確認中。
- **補完**: 入力欄の上に重ねる。幅 288px、上限 8 件。個人が先、`@channel` / `@here` は前方一致したときだけ後ろに出す。
  キャレットの位置には付けない（`textarea` では文字の座標を測れない）。

**この 6 枚は Linux の Chromium で撮っている**（`tools/shoot-ui.mjs` が既定にしている macOS の Chrome ではない）。
レイアウトと色は同じだが、文字のラスタライズが違うので、他の PNG と並べると字面の印象が変わる。オーナーの手元で撮り直す。
