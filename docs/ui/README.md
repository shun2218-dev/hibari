...
- **ミュートは「チャンネルをミュートする」と、その下の「一時的にミュートする」の「1 時間」「明日まで」**。
  ...
  日付の変わり目で終わる期限を「0:00 まで」と書くと、その日もミュートに見えるので、前の日の「いっぱい」と書く（`lib/chat/notifications.ts`）。
...
ADR 0058 のとおり足した。新しい色のトークンは使っていない。アイコンを 4 つ（ホーム・DM・`@`・スマイル）と、
...
- タブは「すべて / DM / メンション / スレッド / リアクション」。サイドバーの幅に 5 つは収まらないので横にスクロールさせ、選んだタブを見える位置に寄せる。
  その下に「未読メッセージ」の切り替え（押している間は緑の枠と地）。
