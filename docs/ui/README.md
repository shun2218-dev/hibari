...
ADR 0052 のとおり、入力欄を Lexical のリッチテキストにした。新しいトークンはない（下線と取り消し線を同じ文字に当てるユーティリティ `underline-strike` を `globals.css` に足しただけ）。
...
- **ツールバーの表示・非表示は、送信の左の「Aa」**（Slack の書式設定アイコン）。隠した状態はそのブラウザに覚える。隠しても記号の入力とショートカットは効く。
