...
- **管理画面からチャットに戻る導線（デスクトップ）**（Phase 6-2 の構築順 6 で判明）: `WorkspaceAdminLayout` の「戻る」はモバイルのヘッダーにしかなく、md 以上では出ない。いまはブラウザの戻る操作に頼っている（ADR 0029）。
- **チャンネルの設定を開く入口と、追加するメンバーを選ぶ画面**（Phase 6-2 の構築順 6 で判明）: `chat/room-settings-dialog.png` はあるが、ルームのヘッダーに開くボタンがない。ダイアログの「メンバーを追加」を押した後に相手を選ぶ画面もない。デザインが足されるまでつながない。
- **DM を始める入口**（Phase 6-2 の構築順 6 で判明）: `chat/dm-dialog.png` はあるが、サイドバーには「チャンネルを作成」しかない。
...
- **設定・管理画面からチャットに戻る導線**（Phase 6-2 の構築順 6 で判明）: 設定（`settings/*.png`）にもワークスペースの管理にも、チャットへ戻る導線がない。モバイルの「戻る」は設定の一覧までで、デスクトップには何もない。
