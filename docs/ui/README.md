...
| 画面 | スクリーンショット | 関連する API |
|---|---|---|
| タイムラインのカード（いちばん小さい・画像なし・画像つき） | `chat/link/link-preview.png`、`chat/link/link-preview-dark.png`、`chat/link/mobile-link-preview.png` | メッセージの `link_previews`、`GET …/link-previews/{id}/urls`（画像とアイコン） |
...
- 説明がなければタイトルとサイト名だけ、アイコンがなければサイト名だけにする（`chat/link/link-preview.png` のいちばん上）。
...
- **大きい画像の出し分けはしない。** ADR 0065 決定 12 は「小さい画像はサムネイル、大きい画像は下に大きく」をデザインで決めることにしていた。
  オーナーが示した Slack の画面は、横長の OGP 画像（1200×630）でも右のサムネイルだったので、それに合わせて**いつもサムネイル**にした。
...
- **会話のメッセージ**は Slack と同じく、アバターの代わりにヘッドフォンのアイコンを置いた行。進行中はアイコンの地と「ライブ」を琥珀にし（Slack は緑）、入っている人の顔と名前、「参加」。
...
- 「ライブ」とアイコンの地は琥珀にした（Slack は緑。hibari の緑は押せるものの色）。Slack の進行中の行は地の色も変わるが、メンションの行の地（琥珀）と紛れるので、行の地は変えていない。
