...
- **ミュートは「チャンネルをミュートする」と、その下の「一時的にミュートする」の「1 時間」「明日まで」**。
  ミュート中は、代わりに「ミュート中」（期限つきなら「今日 18:30 までミュート中」）と「ミュートを解除する」を出す。
...
- **「明日まで」がいつまでかは未決。** 案は「翌日の 9:00 まで」（0:00 にすると、夜にミュートしたときにすぐ切れる）。
  Slack の「明日まで」の時刻は**未確認**。データ層で `muted_until` を求めるときに使うので、構築順 4 の前に決める。
