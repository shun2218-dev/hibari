...
- **設定のダイアログ**: 絵文字のボタン + 文言（100 文字まで。残りの数を下に出す）+ よく使うもの 5 つ + 消える時刻 6 つ。
...
- **「消える時刻」は 6 つの丸いラジオ**（消さない / 30 分 / 1 時間 / 4 時間 / 今日 / 今週）にした。招待リンクの有効期限と同じ部品。
  「今日」「今週」のような相対の期限は、押した時点でクライアントが絶対の時刻にして送る（ADR 0049 決定 5）。
