...
|---|---|---|
| チャンネルで進行中（入っていない）: ヘッダーの「参加」・サイドバーの印・会話のメッセージ（「ライブ」） | `chat/huddle/active.png` | ルームの応答の `huddle`、`huddle.updated` |
| 入っている: ヘッダーの緑のヘッドフォン（押すとハドルのタブを前に出す） | `chat/huddle/joined.png` | — |
...
- **サイドバーの印**は名前の横の琥珀のヘッドフォン。「いま起きていること」なので琥珀にし、押せるようにはしない。
