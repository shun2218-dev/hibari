...
ADR 0051 のとおり、既存のトークンのまま足した（新しいトークンは要らなかった）。本文は保存された記法のまま持ち、表示のときに解釈する。
...
- **リストは記号と番号を出し、段ごとに字下げする**（記号はどの段も `•`。Tailwind に段ごとの記号のユーティリティがなく、任意値は使えない）。
  番号付きリストは書いた番号を出す。
...
- **リストの記号を段で変えない**（Slack は段ごとに `•` `◦` `▪` と変わるが、トークンの外の値を書かないため）。
