...
### Phase 6.17 で足した画面（外部のリンクのプレビュー）

ADR 0065 のとおり足した。新しいトークンは足していない。架空のサイト（`.example`）と、図形だけのモックの画像・アイコン（`web/public/dev/ogp-*.png`・`site-icon-*.png`）で描いた。

| 画面 | スクリーンショット | 関連する API |
|---|---|---|
| タイムラインのカード（横長の画像は上に大きく） | `chat/link/link-preview.png`、`chat/link/link-preview-dark.png`、`chat/link/mobile-link-preview.png` | メッセージの `link_previews`、`GET …/link-previews/{id}/urls`（画像とアイコン） |
| タイムラインのカード（画像なし・横長ではない画像は右のサムネイル） | `chat/link/link-preview-thumbnail.png` | 同上 |
| 本人のカードの「x」 | `chat/link/link-preview-remove.png` | `DELETE …/link-previews/{id}` |
| スレッドのパネルのカード | `chat/link/link-preview-thread.png` | 同上 |
| 入力欄のカード（取れた / 取得中） | `chat/link/composer-link-preview.png`、`chat/link/composer-link-preview-loading.png`、`chat/link/mobile-composer-link-preview.png` | `POST /rooms/{id}/link-previews` |

- **カードの形は Slack に合わせた**（オーナーが示した Slack の画面。ADR 0065 の背景）。左にタイトル（2 行まで）・説明（2 行まで）・サイトのアイコンと名前、右に画像のサムネイル。
  サムネイルはカードの高さに合わせて切り取る（幅 120px。モバイルは 80px）。カードの幅は 600px まで。
- **大きい横長の画像（幅 400px 以上で、幅 ÷ 高さが 1.5 以上）は、上に幅いっぱいで出す**（ADR 0065 の追記。オーナーの要望、2026-09-26）。
  右のサムネイルでは左右が切れて、横長のロゴなどが読めなくなるため。画像は縦横比のまま切らずに出し、その下にタイトル・説明・サイト名を並べる。
  このカードは幅 400px まで（600px いっぱいでは画像だけで画面の半分を占める）。OGP の標準の画像（1200×630）はこちらになる。
- **タイトルは本文の色の太字**にし、リンクであることはホバーの下線で示す（Slack と同じ）。押すと、本文に書かれた URL を別のタブで開く。
- 説明がなければタイトルとサイト名だけ、アイコンがなければサイト名だけにする（部品の story の `components/chat/LinkPreviewCard` の「タイトルだけ」。大きい画像のカードが画面の高さを取るので、画面の写真には写らなくなった）。
- 画像とアイコンの URL を取る前は、同じ大きさの枠だけを置く（読み込みでカードの高さを変えない）。
- **本人の「x」はカードの左の外側**に、ホバーかフォーカスのときだけ出す（Slack と同じ置き場所）。押したらすぐ消える（確認しない。ADR 0065 決定 5）。
- **入力欄のカード**は、入力欄の枠の中で本文の下に出す（Slack と同じ）。1 行目がアイコンとサイト名（太字）、2 行目がタイトル（緑のリンク）。
  取得中はホスト名と回転するリングを出し、2 行目の高さを先に取っておく（取れたときに入力欄の高さが変わらない）。「x」は常に出す。

#### デザインで決めた点（オーナーに確認する）

- ~~**大きい画像の出し分けはしない。**~~ → **出し分ける**（上の「大きい横長の画像」。オーナーの要望、2026-09-26）。
  はじめは、オーナーが示した Slack の画面が横長の OGP 画像（1200×630）でも右のサムネイルだったので、いつもサムネイルにしていた。
  使ってみると横長のロゴが切れて読めなかったので改めた。Slack は大きい画像を文字の下に置くが、オーナーの要望で上にした。
- **並びは、添付とパーマリンクのカード（ADR 0040）の下**にした。ADR 0065 決定 12 は「添付の上」としていたが、本文から出てくるカードどうし（パーマリンクと外部のリンク）を続けて並べる方が、本文との対応が分かりやすい。
- 入力欄のカードのタイトルは緑（押すとそのページを開くリンク）。タイムラインのカードのタイトルは本文の色。Slack の入力欄と投稿後のカードもそう分かれている。

### Phase 6.18a で足した画面（音声のハドル）

ADR 0066 とその追記（A〜D）のとおり足した。新しいトークンは足していない。
オーナーが示した Slack の実物の画面（参加前のプレビュー、別のタブの大きな画面、ハドルのチャット、会話のメッセージの進行中と終了）に合わせて描いた。

**チャットのタブ**

| 画面 | スクリーンショット | 関連する API・イベント |
|---|---|---|
| チャンネルで進行中（入っていない）: ヘッダーの「参加」・サイドバーの印（顔・ヘッドフォン・人数）・会話のメッセージ（「ライブ」） | `chat/huddle/active.png` | ルームの応答の `huddle`、`huddle.updated` |
| 入っていて、ハドルのタブを閉じている: 下のハドルの帯・ヘッダーの緑のヘッドフォン | `chat/huddle/joined.png`、`chat/huddle/joined-dark.png`、`chat/huddle/mobile-joined.png` | — |
| ハドルのチャットをスレッドのパネルで開いた | `chat/huddle/joined-chat.png` | スレッドの API（ADR 0036。親はハドルのメッセージ） |
| 終わったハドルのメッセージ（所要時間・参加した人・返信） | `chat/huddle/ended.png`、`chat/huddle/ended-dark.png` | `message.updated` |
| DM の呼び出し | `chat/huddle/dm-ring.png`、`chat/huddle/mobile-dm-ring.png` | `huddle.ringing`、`POST /huddles/{id}/joining-soon` |
| DM の不在着信と応答なし | `chat/huddle/dm-missed.png` | — |

**ハドルのタブ**（チャットのタブが about:blank に描く。モバイルとタブを開けないときは同じタブの全画面）

| 画面 | スクリーンショット | 関連する API・イベント |
|---|---|---|
| 参加前のプレビュー（開始 / 進行中に参加・マイクのオンとオフ・機器の選択） | `chat/huddle/preview.png`、`chat/huddle/preview-dark.png`、`chat/huddle/preview-mic-menu.png`、`chat/huddle/preview-join-muted.png`、`chat/huddle/mobile-preview.png` | `POST /rooms/{id}/huddle/ice-servers` |
| マイクを使えない | `chat/huddle/preview-mic-denied.png` | — |
| ハドルの画面（参加者のタイル・話している人・ミュート） | `chat/huddle/screen.png`、`chat/huddle/screen-dark.png`、`chat/huddle/screen-crowded.png`、`chat/huddle/mobile-screen.png` | `POST /rooms/{id}/huddle/participants`、`huddle.updated` |
| ハドルのチャット（右のパネル） | `chat/huddle/screen-chat.png` | スレッドの API |
| 自分がミュート / マイクとスピーカーを選ぶ | `chat/huddle/screen-muted.png`、`chat/huddle/screen-device-menu.png` | `PATCH …/participants/{id}` |
| 接続している / 再接続している | `chat/huddle/screen-connecting.png`、`chat/huddle/screen-reconnecting.png` | — |
| DM で相手が「もうすぐ参加する」を押した | `chat/huddle/screen-joining-soon.png` | `huddle.updated` の `joining_soon` |
| 人数の上限 / 切断された | `chat/huddle/screen-full.png`、`chat/huddle/screen-disconnected.png` | 409 `huddle-full`、心拍の ack の `not_found` |

- **ヘッダーのボタン**: 進行中でなければヘッドフォンのアイコンだけ（押すとハドルのタブを開き、プレビューを出す）。進行中は入っている人のアバターと「参加」（押せるので緑の枠）。
  入っていれば緑の地のヘッドフォン（Slack の緑のボタンと同じ）で、押すとハドルのタブを前に出す。抜けるのはハドルの画面の「退出する」から。
  モバイルはアイコンだけ。入れない人（参加していない public ルーム・アーカイブ済み）には出さない（ADR 0066 決定 7）。**ヘッダーが写る画面は、すべてこのアイコンの入った形に撮り直した。**
- **サイドバーの印**は行の右端に、入っている人の顔（2 人まで）と琥珀のヘッドフォンと人数（Slack と同じ）。「いま起きていること」なので琥珀にし、押せるようにはしない。
- **ハドルの帯**は、ハドルのタブを閉じている間、チャットのタブの下の端に全幅で出す（Slack と同じ）。左にルーム名と参加の様子、中央にハドルの画面と同じ操作の列、右に「新しいウィンドウで開く」と「退出する」。
  タブを閉じても抜けない（ADR 0066 追記 C の改め）。モバイルは 2 段にする。
- **会話のメッセージ**は Slack と同じく、アバターの代わりにヘッドフォンのアイコンを置いた行。進行中は行の地・アイコン・「ライブ」を琥珀にし（Slack は緑）、入っている人の顔と名前、「参加」。
  終わったら「ハドルミーティングは終了しました」と「12 分 · あなた、佐藤 直樹、ほか 3 人が参加しました」。下にハドルのチャットの「N 件の返信」。Slack の添え書き（「この静けさをお楽しみください」など）は写さない。
  DM で入らなかった人には「不在着信」、始めた人には「応答なし」。
- **参加前のプレビュー**は Slack と同じ並び: 見出し（「#ルーム でハドルミーティングを開始する」）、自分のタイルとマイクのオンとオフ、マイクとスピーカーの選択、下に「キャンセル」と開始。
  マイクを使えないときはタイルの下に理由を出し、開始を押せなくする。カメラは 6.18b。
- **ハドルの画面**: 上に「#ルーム でハドルミーティングを行う」と人数、中央に参加者のタイル（人数で列の数と高さを変える）、下にマイク（「⌄」で機器の選択）・ハドルのチャット・右端の「退出する」（danger）。
  話している人はタイルの枠を琥珀に、ミュートしている人は右下にマイクの斜線。背景のテーマ・招待・挙手・リアクション・canvas は作らない（ADR 0066 追記 C）。
- **DM の呼び出し**は、md 以上で右下、モバイルで上に重ねる。ボタンは「もうすぐ参加する」と「参加」だけ（拒否はない）。

#### デザインで決めた点（オーナーに確認する）

- 「ライブ」とアイコンの地は琥珀にした（Slack は緑。hibari の緑は押せるものの色）。進行中の行は、Slack と同じく行ごと地の色を付ける（琥珀の地。オーナーの指摘で付けた）。
- 進行中のメッセージの見出しは「ハドルミーティング」だけにした。Slack は自分が入っていると「ハドルミーティングに参加しました」になるが、入っていない人から見た文言は確かめていない。
- 呼び出しの置き場所（md 以上は右下、モバイルは上）。Slack の実物の置き場所は確かめていない。
- ハドルの画面の背景は、トークンの背景色（`--color-background`）のまま。Slack のようなテーマの絵は入れていない。
