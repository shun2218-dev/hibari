...
**ADR で決めること** ← 完了（ADR 0061。オーナーの確認待ち）
...
**構築順**（PR を分ける）
1. 設計: ADR 0061 ← オーナーの確認待ち
2. 基盤: pg_bigm 入りの Postgres イメージ（compose・CI・`docs/deploy.md`）と、拡張・索引のマイグレーション
...
- [ ] 2 文字の日本語で索引が使われる（`EXPLAIN` を見るテスト）
- [ ] 大文字小文字と全角半角の違いを吸収する（`Deploy` / `ｄｅｐｌｏｙ` で `deploy` が見つかる）
