...
**オーナーの要望**（2026-09-22）: Slack と同じく、スレッドごとにミュートできるようにする。6.14a の後に、別の ADR で扱う。
番号は 6.14b の後だが、**6.14b より先に作る**。6.14b の通知の規則（ADR 0055 決定 1 の「参加しているスレッドの返信」）が、
スレッドのミュートを読むことになるため（6.14a を先にしたのと同じ理由）。

**ADR で決めること**
- Slack の実物の挙動（メニューの文言、「フォローをやめる」との違い、ミュートしたスレッドの未読とスレッドの一覧での見え方）。推測せず、実物か公式ヘルプで確かめる
- 持ち方（案: スレッドの参加者の行（ADR 0036 の `thread_members`）に列を足す。参加していないスレッドはそもそも通知しない）
- スレッドの未読（`thread_seq`）とスレッドの一覧の見せ方を、チャンネルのミュートと同じく「見せ方だけを変える」にするか
- 同期（ADR 0055 決定 5 と同じく、本人宛てのイベントと再接続の取り直し）

**DoD**（ADR を書くときに確定する）
- [ ] スレッドをミュートすると、その返信は通知の対象にならず、スレッドの一覧で強調されない
- [ ] 設定の変更が本人のほかのタブにも揃い、切断中の変更も再接続で揃う

### Phase 6.14b — ブラウザ通知

**ADR で決めること**
- 通知する条件（ADR 0055 決定 1 の規則。タブが見えているときは出さない）。ミュートしたチャンネルのメンションで通知を出すかは、Slack の実物で確かめる
- 複数のタブで同じ通知を 1 回だけ出す方法（案: 通知の `tag` にメッセージの ID を入れて置き換えさせ、どこかのタブが見えているかは Web Locks で判定する。ADR 0024 と同じ仕組み）
- 通知の許可をいつ求めるか（案: サイドバーの上に「通知を有効にする」の帯を出し、押したときに求める。ブラウザは操作をきっかけにしない許可の要求を拒む）
- `Delivery` との関係（ブラウザ通知は既存の WebSocket の配信をクライアントが通知に変えるだけで、配信先は増やさない案。Push 通知を足すときの形は崩さない）
- 押したときにそのメッセージへ飛ぶ（6.11b の仕組み）
...
**ADR で決めること**
- メニューに何を置くか（ホーム・DM・アクティビティのほか、既存のスレッドの一覧や 6.12 の保存をどこに置くか）。モバイルでの出し方
- アクティビティに並べるもの（6.14 の通知の対象と同じか。メンション・DM・参加しているスレッドの返信・自分のメッセージへのリアクションなど）
- 既読の持ち方（1 件ずつ既読にするか、開いたらまとめて既読か）と、未読の件数をメニューに出すか
- 一覧の API（カーソルでのページング）と、リアルタイムでの追加（WebSocket のイベント）。読めなくなったルームのメッセージを出さないこと
- 押したら 6.11b の仕組みでそのメッセージへ飛ぶ

**DoD**（ADR を書くときに確定する）
- [ ] 左のメニューでサイドバーの中身が切り替わり、今いる画面が分かる
- [ ] アクティビティに通知の対象のメッセージがメッセージ単位で並び、新しいものがリアルタイムに加わる
- [ ] 読めなくなったルームのメッセージはアクティビティに出ない（API のテスト）
- [ ] 押すとそのメッセージへ飛ぶ
