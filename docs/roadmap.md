...
- ツールバー（リンクのボタンは置かない）・ショートカット（Slack の公式の一覧に合わせる）・記号の入力。貼り付けはテキストとして入れる
- Enter で送信・Shift + Enter で改行、16 行の上限（`--composer-max-lines`）はそのまま

**構築順**（PR を分ける）
1. 設計（ADR 0052） ← このフェーズの最初の PR
2. デザイン: ツールバー（デスクトップとモバイル）、入力欄の中の書式とメンションのチップ、キャレットの位置の補完、編集欄。story に描いて `docs/ui/` に足し、オーナーに見てもらう
3. エディタの芯: `components/chat/editor/` に読み込み・書き出し・メンションのノード。往復のテストを厚くする
4. つなぎ込み: 入力欄と編集欄を置き換え、ツールバー・ショートカット・記号の入力・補完・貼り付け。`toWireBody` / `toInputBody` を消す
