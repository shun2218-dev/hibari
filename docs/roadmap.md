...
2. デザイン: ツールバー（デスクトップとモバイル）とリンクを入れる画面、入力欄の中の書式とメンションのチップ、キャレットの位置の補完、編集欄、本文の文字付きのリンク。story に描いて `docs/ui/` に足し、オーナーに見てもらう
3. 記法の追加: `<URL|文字>` の解釈と表示（`body-format.ts` / `MessageBody`。`testdata/format/mentions.json` に読み方がずれやすい例を足す）
4. エディタの芯: `components/chat/editor/` に読み込み・書き出し・メンションのノード。往復のテストを厚くする
5. つなぎ込み: 入力欄と編集欄を置き換え、ツールバー・ショートカット・記号の入力・補完・貼り付け。`toWireBody` / `toInputBody` を消す
