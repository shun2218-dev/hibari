...
2. デザイン: ヘッダーのボタン、サイドバーの印、参加前のプレビュー、別のタブのハドルの画面（参加者・話している人・ミュート・ハドルのチャット）とハドルの帯、会話のメッセージ（進行中・終了・不在着信）、DM の呼び出し、マイクの許可とエラーと再接続、モバイル。story に描いて `docs/ui/` に足し、オーナーに見てもらう（ADR 0066 の追記で、サイドバーの窓をやめて別のタブにした） ← 描いた（`docs/ui/README.md` の「Phase 6.18a で足した画面」。オーナーの確認待ち）
3. サーバー: `platform/sfu`・`platform/turn`、設定、マイグレーション、authz、入る・受ける・抜ける・ミュートの API、掃除のジョブ、ルール 8 の外し方、ルームの応答のハドル ← SFU と TURN と設定は #195。DB・状態・サービス・掃除・ルール 8・イベントの型は次の PR。HTTP の API と心拍はその次
4. WebSocket と `docs/events.md`（`huddle.updated` / `huddle.ringing` / `huddle.left`、心拍）
5. Web: 接続の状態の管理、受けるトラックの出し入れ、話している人の判定、ハドルの窓、呼び出し、ショートカット
6. 実物の確認（開発用の Cloudflare のアプリで）

**DoD**
- [ ] DM とチャンネルでハドルを始めて入ると、互いの声が聞こえる（実物。2 つのブラウザと、TURN を通る違うネットワークで）
- [ ] 2 人が同時に始めても、ルームのハドルは 1 つになる（並行テスト）
- [ ] 最後の人が抜けるとハドルが終わり、会話のメッセージが「終了」になる。再接続の同期（`after_change_seq` とルームの取得）でもそろう
- [ ] タブを閉じる・心拍が途絶えると、ほかの人の画面から消える（`Clock` を固定したテスト）。掃除のジョブの goroutine が残らない
- [ ] キック・private のルームからの削除・アーカイブ・セッションの失効で、その人がすぐにハドルから外れ、聞こえなくなる（テストと実物）
- [ ] ルームに投稿できない人は、ICE サーバーの取得・入る・受けるのどれもできない（API で 403 / 409）。同じハドルにいない人の音声は受けられない
- [ ] 別の端末から入ると、前の端末の参加が外れる
- [ ] DM では相手に呼び出しが届き、60 秒・参加・「もうすぐ参加する」・終了で止まる。「もうすぐ参加する」はハドルにいる人の画面に出る。ミュートした DM では呼び出さない
- [ ] DM で相手が入らないまま終わると、入らなかった人には不在着信として見え、DM に未読が付く
- [ ] 終わったハドルのメッセージに、所要時間と参加した人（2 人まで名前、それより多ければ「ほか N 人」）が出る
- [ ] 21 人目は入れない（409）
- [ ] ハドルのメッセージにスレッド（ハドルのチャット）が付き、ハドルの画面とチャットのタブの両方で同じスレッドを読み書きできる。ほかのシステムメッセージには付けられない（ADR 0066 追記 A）
- [ ] 参加前のプレビューでマイクとスピーカーを選べ、選んだものを次も使う。マイクを許可しないときはプレビューで知らせる（追記 B）
- [ ] ハドルの画面は別のタブに出る。そのタブを閉じても抜けず、チャットのタブの下にハドルの帯が出て、そこから開き直せる。タブを開けないときとモバイルでは同じタブに全画面で出る（追記 C）
- [ ] Cloudflare の設定がなくてもサーバーは起動し、ハドルだけが無効になる
- [ ] ログに SDP・TURN の認証情報・アプリの秘密が出ない
