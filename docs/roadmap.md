...
**ADR で決めること** ← ADR 0058 で提案中
- メニューに何を置くか → ホーム・DM・アクティビティ・後で。開いているメニューは URL に持つ。一覧をどの列に出すかとモバイルの出し方はデザインで決める
...
- DM の一覧 → ルーム一覧の DM の行に `last_message` を足す

**構築順**（PR を分ける）
1. 設計: ADR 0058
2. デザイン: 左のメニュー（デスクトップとモバイル）、アクティビティの一覧、DM の一覧
3. サーバー: アクティビティの API・未読の件数・リアクションのイベント、ルーム一覧の `last_message`、通知の規則の共有のテストデータ
4. Web: メニューと URL、アクティビティ・DM の一覧、リアルタイムの追加、「後で」の移動
...
- [ ] 押すとそのメッセージへ飛ぶ
- [ ] ルームを読むと、アクティビティのそのメッセージも既読になり、メニューのバッジが減る
- [ ] 自分のメッセージにリアクションが付くと、アクティビティに加わる
- [ ] DM の一覧に、相手ごとの最後のメッセージが新しい順に並ぶ
- [ ] 通知の規則のテストデータを、Go と Web のテストが両方読んでいる
