...
Phase 6.4  システムメッセージ
Phase 6.5  スレッド
Phase 6.6〜6.16  Slack と同じ使い心地にする（チャンネルにも投稿・リアクション・離席とステータス・プロフィール・書式・リンク・ピン留め・メンション・通知・アーカイブ・検索）
Phase 7〜  任意
```
...
## Phase 6.6 — チャンネルにも投稿する ← 完了（2026-09-19、オーナーの確認待ち）
...
**DoD**
- [x] 「チャンネルにも投稿する」を付けた返信が、スレッドとチャンネルの両方に出る。付けない返信はチャンネルに出ない（`TestSendThreadReplyAlsoInChannel`、`views.test.ts` / `workspace-screen.test.tsx` の ADR 0039）
- [x] 切断中に流された返信も、再接続の同期でチャンネルとスレッドの両方に揃う（`store.test.ts` の「puts a flagged reply that arrived while disconnected...」。オーナーによる compose とブラウザでの確認は未）
- [x] 未読とサイドバーの並びが、ADR で決めたとおりに動く（`TestSendThreadReplyAlsoInChannel`、`messages.test.ts` / `store.test.ts` の ADR 0039）
...
**ADR で決めること**
- テーブル（案: `message_reactions(message_id, user_id, emoji)` を主キーにして、二重に付けられないことと付け外しの冪等性を DB で保証する）
- 同期（案: 付け外しのたびにメッセージの `change_seq` を進め、集計したリアクションをメッセージの形に含める。`message.updated` と同じ流れに乗せ、同期の経路を増やさない）
- 1 つのメッセージに付けられる種類の上限、絵文字の文字列の検証
- 絵文字のピッカー（ライブラリを入れるか）
- 誰が付けたかの表示（ホバーで名前）
...
**ADR で決めること**
- Redis の持ち方（案: 今のインスタンスごとのフィールドに「画面を見ている接続の数」も持たせる。状態の判定と `presence.changed` の publish は、今と同じく 1 つの Lua スクリプトで行い、順序を保つ。ADR 0016）
- クライアントが「見ている / 見ていない」を送るイベント（visibilitychange・focus・無操作のタイマー）と、送りすぎないための間引き
- `presence.changed` の形（`online` の真偽値から状態へ。既存のクライアントとの互換）
- 手動の離席とカスタムステータスの変更を、本人のほかのタブと、同じワークスペースのメンバーにどう届けるか。再接続の同期で揃える経路
- カスタムステータスの期限切れを、誰がいつ消すか（読むときに `Clock` で判定して返さない案。掃除のジョブが要るか）
- 文言の長さの上限、絵文字のピッカー（6.7 のリアクションのものを使う）
- 離席の人への通知（6.14）との関係
- アウトラインのドットの色（既存のトークンで描けるか。足りなければトークンを確認する）
...
**ADR で決めること**
- chat の API が他人の email を返すのはこれが初めてになる。どの API で返すか（メンバーの一覧に足すか、1 人分を取る API を足すか）
- 未検証の email を出すか
- 管理の入口を出すかどうかを、authz の判定結果としてどう返すか（クライアントに `canManage` を再実装させない）
- デスクトップはポップオーバー、モバイルは下から出るシート

**DoD**
- [ ] メッセージ・スレッド・メンバーパネルのアイコンと名前から、カードが開く
...
**ADR で決めること**
- 記法（Slack の mrkdwn に近いものか、Markdown のサブセットか）と、保存する形（本文はそのまま保存し、表示のときに解釈する案）
- 解釈をどこで行うか（案: Web。サーバーは本文をそのまま返す）。XSS を起こさない描画の仕方（HTML として埋め込まない）
- 入力欄の補助（ショートカット・ツールバー）をどこまで作るか
- システムメッセージと既存のメッセージへの影響
...
**ADR で決めること**
- URL の形（案: `/w/{workspaceID}/r/{roomID}?m={messageID}`。スレッドの返信ならパネルも開く）
- 指定したメッセージの前後を取る API（今は `before_seq` / `after_seq` だけで、古いメッセージまで飛べない）
- カードの中身をまとめて取る API（ADR 0020 のアバターと同じく、画面に出るものを 1 回で取る）。別のワークスペースのリンクの扱い
- 新着の線（ここから未読）と、未読の位置へ飛ぶボタン。既読の位置はすでに持っているので、同じ飛ぶ仕組みに乗せる

**DoD**
- [ ] コピーしたリンクを開くと、古いメッセージでもそこまで飛んで強調される。スレッドの返信ならパネルが開く
- [ ] 本文にリンクを貼るとカードが出て、長い本文は畳まれ、広げる・畳むができる
- [ ] 読めないルームのメッセージのリンクは、中身を返さない（API のテスト）
- [ ] 未読のあるルームを開くと新着の線が出て、未読の位置へ飛べる

---

## Phase 6.12 — ピン留めと保存

**目的**: チャンネルにメッセージをピン留めする機能と、自分用にメッセージを保存する（後で見る）機能を足す。

**ADR で決めること**
- ピン留めはルームで共有、保存は本人だけ。同期の経路がそれぞれ違う（ピン留めはルームの `change_seq`、保存は本人宛てのイベント）
- ピン留めできる人（案: 投稿できる人なら誰でも。Slack と同じ）と、ピン留めの数の上限
- 保存したメッセージのルームから外れたときや、メッセージが削除されたときの見え方
...
**ADR で決めること**
- 本文での表し方（案: `<@userID>` で保存し、表示のときに名前へ置き換える。名前を変えても古くならない）
- **件数の数え方。** `@channel` は 1 回の送信でルームの全員の件数を増やすことになる。大きなルームでも送信が重くならない形を比べる（メンバーの行をまとめて 1 回の UPDATE で増やす / メンションを別のテーブルに積んでそこから数える）
- `@here` の対象（送った瞬間の presence。TTL で持っているので目安になる）
- `@channel` / `@here` を使える人を絞るか（案: 最初は誰でも。制限はワークスペースの設定として後から足せる形にする）
- スレッドの中の `@channel` の対象（ルームの全員か、スレッドの参加者だけか）
- ルームのメンバーでない人をメンションしたとき（案: 知らせない。入力欄で警告する）
- 編集でメンションを足した・消したときの件数
...
**ADR で決めること**
- ミュートの意味（案: Slack と同じく、未読を強調しないがメンションは数える）。ミュートは本人の設定で、ほかのタブにも届ける
- 通知する条件（メンション・DM・参加しているスレッドの返信。タブが見えているときは出さない）と、ユーザー設定
- 複数のタブで同じ通知を 1 回だけ出す方法（ADR 0024 の Web Locks と同じ考え方）
- 通知の許可をいつ求めるか
- `Delivery` との関係（ブラウザ通知は既存の WebSocket の配信をクライアントが通知に変えるだけで、配信先は増やさない案。Push 通知を足すときの形は崩さない）
...
**ADR で決めること**
- できる人（案: admin 以上）。`is_default` のルームと DM は対象外にするか
- アーカイブ中に禁止する操作（投稿・編集・リアクション・参加など）と、その判定を authz に集める形
- 削除したときの添付ファイルの掃除（既存の掃除ジョブとの関係）と、メッセージのリンク・ピン留め・保存の見え方
- 購読中のユーザーへの即時の反映（絶対ルール 8 と同じく、サーバー側で購読を解除してイベントを配信する）
- アーカイブを戻せるか
...
**ADR で決めること**
- 日本語の検索の方式（Postgres 標準の全文検索は日本語を分かち書きできない。`pg_trgm` / `pg_bigm` / 別の検索エンジンを比べる。ローカルの compose と本番の Fly.io で同じものを動かせるか）
- 結果の authz（見る人が読めるルームのメッセージだけ。public ルームは参加していなくても読める）と、それを N+1 にならずに絞る方法
- 絞り込み（ルーム・送信者・日付など）とカーソル方式のページング
- 削除・編集されたメッセージの扱い、スレッドの返信の扱い
- 結果を押したら 6.11 の仕組みでそのメッセージへ飛ぶ。一致した部分の強調は 6.10 の仕組みに乗せる
