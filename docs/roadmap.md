...
**DoD**
- [ ] URL を貼って送ると、カードがリアルタイムに全員の画面に現れ、再接続の同期（`after_change_seq`）でも揃う
- [ ] 本人がカードを消すと全員の画面から消え、編集しても戻らない。本人以外は消せない（API で 403）
- [ ] 内部のアドレス（ループバック・プライベート・リンクローカル・メタデータ・`fdaa::/16`・IPv4 を埋め込んだ IPv6）と、そこへのリダイレクトを取りに行かない
- [ ] 画像は自前のストレージから出し、見る人のブラウザは相手のサイトにアクセスしない。読めないルームの画像の URL は取れない
- [ ] 同じ URL を 30 分以内に貼っても、相手のサイトには 1 回しかアクセスしない
- [ ] 入力欄に URL を書くと送る前にカードが出て、「x」で消したものは付かずに送られ、編集しても戻らない。入力欄で取れていれば、送った時点でカードが付いている
- [ ] 入力欄の API は、投稿できないルームでは使えず、回数の上限を超えると 429 になる。カードにならない理由を区別しない
- [ ] カードにサイトのアイコンが出る（自前のストレージから。SVG は出さない）
- [ ] 主なサイトを実物で取った結果を ADR 0065 に残した
- [ ] 取得のジョブの goroutine が、サーバーを止めたときに残らない
