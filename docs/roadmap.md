...
**DoD**
- [ ] curl だけで ワークスペース作成 → 招待 → 別ユーザーが参加 → ルーム作成 → 参加 が通る
- [ ] 権限表のすべてのセルにテストがある（許可と拒否の両方）
- [ ] admin が他の admin を降格もキックもできない
- [ ] `max_uses = 1` の招待に 50 goroutine が同時に参加しても、成功は 1 件だけ
- [ ] owner の譲渡を並行で実行しても、owner が 0 人や 2 人にならない
- [ ] 同じ 2 人の DM を並行で作成しても 1 つにしかならない
...
**DoD**
- [ ] curl だけで 送信 → 履歴取得 が通る
- [ ] 同じ client_msg_id で 2 回送っても、メッセージは 1 件しか増えず、seq も消費されない
- [ ] 50 goroutine で同じルームに同時送信しても、seq に欠番も重複もない
- [ ] public ルームを非メンバーが読めて、書けない
- [ ] 別ルームのメッセージへの返信が DB 制約で拒否される
...
**DoD**
- [ ] curl で 発行 → 直接 PUT → complete → メッセージに添付 → GET URL の取得 が通る
- [ ] 上限を超えるサイズの PUT がストレージ側で拒否される
- [ ] ルームのメンバーでないユーザーは GET URL を取得できない
- [ ] 他人がアップロードした添付を自分のメッセージに付けられない
- [ ] 期限切れの pending が掃除される（Clock を進めるテスト）
...
**DoD**
- [ ] `/dev/preview` で全画面・全状態が再現できる（6-1）
