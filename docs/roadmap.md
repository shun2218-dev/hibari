...
2. デザイン: ヘッダーのボタン、サイドバーの印、参加前のプレビュー、別のタブのハドルの画面（参加者・話している人・ミュート・ハドルのチャット）、会話のメッセージ（進行中・終了・不在着信）、DM の呼び出し、マイクの許可とエラーと再接続、モバイル。story に描いて `docs/ui/` に足し、オーナーに見てもらう（ADR 0066 の追記で、サイドバーの窓をやめて別のタブにした） ← 描いた（`docs/ui/README.md` の「Phase 6.18a で足した画面」。オーナーの確認待ち）
...
- [ ] ハドルの画面は別のタブに出て、そのタブを閉じると抜ける。タブを開けないときとモバイルでは同じタブに全画面で出る（追記 C）
