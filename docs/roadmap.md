...
Phase 6.4  システムメッセージ
Phase 6.5  スレッド
Phase 6.6〜6.15  Slack と同じ使い心地にする（チャンネルにも投稿・プロフィール・リアクション・書式・リンク・ピン留め・メンション・通知・アーカイブ・検索）
Phase 7〜  任意
```
...
## Phase 6.6〜6.15 — Slack と同じ使い心地にする
...
- 本文の書式（6.9）は、URL・パーマリンク・メンションを本文から見つける仕組みも兼ねる。先に作っておくと、6.10 と 6.12 で解釈を作り直さずに済む。
- 指定したメッセージへ飛ぶ仕組み（6.10）は、ピン留め・保存（6.11）と検索（6.15）の結果を押したときにも使う。
- プロフィールのカード（6.7）は、メンションの `@名前` を押したときにも開く。

```
Phase 6.6   チャンネルにも投稿する
Phase 6.7   プロフィールのカード
Phase 6.8   絵文字のリアクション
Phase 6.9   本文の書式
Phase 6.10  メッセージへのリンク（コピー・飛ぶ・カード・新着の線）
Phase 6.11  ピン留めと保存
Phase 6.12  メンション
Phase 6.13  ミュートとブラウザ通知
Phase 6.14  チャンネルのアーカイブと削除
Phase 6.15  検索
```
...
**ADR で決めること**
- チャンネルに流した返信を、チャンネルの未読（`user_seq`）とサイドバーの並びに数えるか（案: 数える。チャンネルに出ている以上、チャンネルの発言として扱う）
- フラグを送信後に変えられるか（案: 変えられない）
- 親や返信が削除されたときのチャンネル側の見え方（ADR 0038 との整合）
...
**オーナーと確定した内容**（2026-09-19）
- 開く場所はアイコンと名前の両方（メッセージ、スレッド、メンバーパネル。6.12 のメンションの `@名前` も）
...
本文から「特別な部分」を見つける仕組みを 1 つにする。6.10 のパーマリンク、6.12 のメンション、6.15 の検索の強調も、この仕組みに乗せる。
...
- 入力欄に `@` の補完。本文の `@名前` を押すとプロフィールのカード（6.7）が開く
...
- 結果を押したら 6.10 の仕組みでそのメッセージへ飛ぶ。一致した部分の強調は 6.9 の仕組みに乗せる
