...
2. デザイン: 各書式・コードブロック・引用・リスト・リンクの見た目と、スレッドの一覧・リンクのカードでの見え方。story に描いて `docs/ui/` に足し、オーナーに見てもらう
3. サーバー: `internal/chat/mention` でコードの範囲のトークンを除外する（共通の JSON のテスト）
4. Web: 解釈（`format.ts`）と描画。`MessageBody` の置き換え、スレッドの一覧とリンクのカード、`toWireBody` / `toInputBody` のコードの除外
...
**ADR で決めること**
- ライブラリ（ADR 0051 の時点の推しは Lexical。着手するときに改めて比べる）
- ツールバーとショートカットをどこまで作るか
- 日本語の IME、メンションの補完、編集モードの移し替え。16 行の上限（`--composer-max-lines`。ADR 0048）の引き継ぎ
- 送る本文は ADR 0051 の記法のテキストにする（保存の形は変えない）

**DoD**（ADR を書くときに確定する）
- [ ] 入力欄で付けた書式が、6.10a の記法のテキストとして送られ、同じ見た目で表示される
- [ ] 日本語の IME で変換している間に、書式やメンションの補完が壊れない
