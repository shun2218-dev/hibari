...
- ピン留めできる人と上限 → `authz.CanPinMessage`（いまは投稿できる人）、100 件。システムメッセージ `message_pinned`
...
2. デザイン: メッセージのピンの印とメニュー、ピン留めのシステムメッセージ、ピン留めの一覧、「後で」の一覧（3 つのタブ・読めない行・外すときの確認）と入口。story に描いて `docs/ui/` に足し、オーナーに見てもらう
   ← 描いた（`docs/ui/README.md` の「Phase 6.12 で足した画面」。オーナーの確認待ち）
...
- [ ] 同じルームで多数の goroutine が同時にピン留めしても、100 件を超えない
