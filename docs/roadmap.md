...
2. デザイン: ヘッダーのボタン、サイドバーの印、ハドルの窓（参加者・話している人・ミュート）、会話のメッセージ（進行中・終了）、DM の呼び出し、マイクの許可とエラーと再接続、モバイル。story に描いて `docs/ui/` に足し、オーナーに見てもらう ← 描いた（`docs/ui/README.md` の「Phase 6.18a で足した画面」。オーナーの確認待ち）
