...
Phase 6.17 外部のリンクのプレビュー                              ← 完了
Phase 6.18 ハドル（6.18a 音声 / 6.18c 一覧・ハドル中の印・リンク / 6.18b カメラと画面共有）
Phase 7〜  任意
```
...
5. Web: 修飾子の解釈と整形・結果の画面・結果から飛ぶ ← 完了
...
- [x] 修飾子とフィルターの UI が同じ状態を編集する（`lib/chat/search/search-query.test.ts` の往復のテスト 9 通りと、`workspace-screen.test.tsx` の「フィルターで送信者を選ぶと、入力欄の修飾子にも出る」）
- [x] 結果から元のメッセージへ飛べる（`lib/chat/views/search.test.ts`。スレッドの返信は `t=` を付けて、飛んだ先でスレッドが開く）

---

## Phase 6.17 — 外部のリンクのプレビュー ← 完了（2026-09-25）

**目的**: 本文に URL を貼ると、そのページの OGP（画像・タイトル・説明）のカードを出す（Slack・LINE・X と同じ）。オーナーの要望（2026-09-25）。

**オーナーと確定した内容**（2026-09-25）
- カードは既定で出す。**投稿した本人だけ**がカードを消せる。消すときに確認のダイアログは出さない
- 振る舞いは一般的なやり方（Slack・LINE・Discord）に合わせる: 投稿した時点の内容をメッセージに固定し、サイト側が変わっても投稿済みのカードは変えない。画像は相手のサイトから直接読ませない
- 送る前にも入力欄にカードを出し、そこで消せる。カードにはサイトのアイコンも出す（どちらも Slack と同じ）

**ADR で決めること** ← 完了（ADR 0065。オーナーの確認: 2026-09-25）
- 取得の時期と保存の形（投稿の後に非同期で取り、`change_seq` と `message.updated` で配る。URL ごとの結果を 30 分使い回す）
- 展開する URL の規則（Slack に合わせて、リンクが 5 つより多いメッセージは展開しない。ヘルプにある「1 時間以内に共有した URL は展開しない」は、オーナーが実物で確かめて採らないことにした）
- 画像の扱い（取得したときに自前のストレージへ写し、署名付き URL で出す）
- SSRF の対策（接続する IP の検査、リダイレクト、サイズと時間の上限）
- 本人が消す API と、編集で戻さないこと

**構築順**（PR を分ける）
1. 設計: ADR 0065、ERD、アーキテクチャの図 ← 完了
2. デザイン: カード（画像なし / 小さい画像 / 大きい画像 / 長い説明 / アイコンなし）、本人の「x」、入力欄のカード（取得中 / 取れた / 消す）、モバイル、スレッドのパネル。story に描いて `docs/ui/` に足し、オーナーに見てもらう ← 完了（`docs/ui/README.md` の「Phase 6.17 で足した画面」）
3. サーバー: `platform/safehttp`、メタデータとアイコンの解析、マイグレーション、取得のジョブ、入力欄の API、消す API・画像とアイコンの URL の API、掃除 ← 完了（前半 #190: 取得と解析、後半: DB・ジョブ・API）
4. WebSocket と `docs/events.md` ← 完了（構築順 3 の後半と一緒に入れた。専用のイベントはなく `message.updated` に乗る）
5. Web: カードの表示、画像とアイコンの取得、消す操作、入力欄のカード ← 完了

**DoD**
- [x] URL を貼って送ると、カードがリアルタイムに全員の画面に現れ、再接続の同期（`after_change_seq`）でも揃う（`TestLinkPreviewAfterSend`、2 台目のインスタンスに届く `TestWSDeliversLinkPreviews`、`workspace-screen.test.tsx` の「投稿の後に取れたカードは message.updated で現れる」）
- [x] 本人がカードを消すと全員の画面から消え、編集しても戻らない。本人以外は消せない（API で 403）（`TestRemoveLinkPreview`、`TestLinkPreviewAPI`、`workspace-screen.test.tsx` の「x」）
- [x] 内部のアドレス（ループバック・プライベート・リンクローカル・メタデータ・`fdaa::/16`・IPv4 を埋め込んだ IPv6）と、そこへのリダイレクトを取りに行かない（`safehttp_test.go`、`fetch_test.go`。compose の実物でも、メタデータの IP と localhost は `preview: null` になった）
- [x] 画像は自前のストレージから出し、見る人のブラウザは相手のサイトにアクセスしない。読めないルームの画像の URL は取れない（`TestLinkPreviewURLsAuthz`、`workspace-screen.test.tsx` の「自前のストレージの署名付き URL で読む」）
- [x] 同じ URL を 30 分以内に貼っても、相手のサイトには 1 回しかアクセスしない（`TestLinkPreviewReuse`）
- [x] 入力欄に URL を書くと送る前にカードが出て、「x」で消したものは付かずに送られ、編集しても戻らない。入力欄で取れていれば、送った時点でカードが付いている（`TestLinkPreviewSuppressedInComposer`、`TestPreviewLink`、`workspace-screen.test.tsx` の入力欄の 2 件）
- [x] 入力欄の API は、投稿できないルームでは使えず、回数の上限を超えると 429 になる。カードにならない理由を区別しない（`TestPreviewLink`、`TestPreviewLinkAPI`）
- [x] カードにサイトのアイコンが出る（自前のストレージから。SVG は出さない）（`fetch_test.go`、`meta_test.go`、`workspace-screen.test.tsx`）
- [x] 主なサイトを実物で取った結果を ADR 0065 に残した（決定 9 の追記）
- [x] 取得のジョブの goroutine が、サーバーを止めたときに残らない（`TestRunLinkPreviewsStops`）

---

## Phase 6.18 — ハドル（通話）

**目的**: Slack のハドルミーティングのように、DM とチャンネルで通話できるようにする。オーナーの要望（2026-09-26）。

**オーナーと確定した内容**（2026-09-26）
- 外部のサービスは Cloudflare Realtime の **SFU と TURN** を使う（RealtimeKit と LiveKit は採らない）。合図・参加の状態・誰が入れるかは Go が持つ
- 範囲は Slack のハドルと同じく **DM とチャンネルの両方**
- **音声を先に作る（6.18a）。カメラと画面共有は後（6.18b）**。いちばん難しい通話の状態の管理を、音声だけで先に固める

### Phase 6.18a — 音声のハドル

**ADR で決めること** ← 完了（ADR 0066。オーナーの確認: 2026-09-26）
- DM の 1 対 1 も SFU を通すか（P2P にするか）
- アプリの秘密の置き場所と、SFU の API を誰が呼ぶか
- 状態の置き場所（履歴は Postgres、いま入っている人は Redis。ルール 5）
- 生きているかの判定（心拍・期限切れの見つけ方。Cloudflare は切れたことを知らせない）
- 1 人が入れるハドルの数、別の端末から入ったとき
- 誰が入れるか（authz）と、権限が変わったときにすぐ外す方法（ルール 8）
- DM の呼び出し、会話に残すメッセージ、イベントと再接続の同期（ルール 4）
- 開発とテストで Cloudflare をどう扱うか（ローカルで動く版がない）

**構築順**（PR を分ける）
1. 設計: ADR 0066、ERD、アーキテクチャの図 ← 完了
2. デザイン: ヘッダーのボタン、サイドバーの印、参加前のプレビュー、別のタブのハドルの画面（参加者・話している人・ミュート・ハドルのチャット）とハドルの帯、会話のメッセージ（進行中・終了・不在着信）、DM の呼び出し、マイクの許可とエラーと再接続、モバイル。story に描いて `docs/ui/` に足し、オーナーに見てもらう（ADR 0066 の追記で、サイドバーの窓をやめて別のタブにした） ← 完了（#194。`docs/ui/README.md` の「Phase 6.18a で足した画面」。オーナーの確認済み）
...
- [x] キック・private のルームからの削除・アーカイブ・セッションの失効で、その人がすぐにハドルから外れ、聞こえなくなる（テストと実物）（`TestHuddleRemovedOnAccessChange`。実物はキック）
- [x] ルームに投稿できない人は、ICE サーバーの取得・入る・受けるのどれもできない（API で 403 / 409）。同じハドルにいない人の音声は受けられない（`TestJoinHuddleAuthz`・`TestSubscribeHuddle`）
- [x] 別の端末から入ると、前の端末の参加が外れる（`TestJoinHuddleFromAnotherDevice`）
- [x] DM では相手に呼び出しが届き、60 秒・参加・「もうすぐ参加する」・終了で止まる。「もうすぐ参加する」はハドルにいる人の画面に出る。ミュートした DM では呼び出さない（`TestDMHuddle`、web の `store/huddles.test.ts`・`huddle-call.test.tsx`、実物）
- [x] DM で相手が入らないまま終わると、入らなかった人には不在着信として見え、DM に未読が付く（`TestDMHuddle`、web の `views/huddles.test.ts`、実物）
- [x] 終わったハドルのメッセージに、所要時間と参加した人（2 人まで名前、それより多ければ「ほか N 人」）が出る（web の `views/huddles.test.ts`）
- [x] 21 人目は入れない（409）（`TestJoinHuddleFull`・`TestConcurrentJoinsRespectLimit`。409 `huddle-full` への変換は `httpx/problem.go`）
- [x] ハドルのメッセージにスレッド（ハドルのチャット）が付き、ハドルの画面とチャットのタブの両方で同じスレッドを読み書きできる。ほかのシステムメッセージには付けられない（ADR 0066 追記 A）（`TestHuddleThread`、実物）
- [x] 参加前のプレビューでマイクとスピーカーを選べ、選んだものを次も使う。マイクを許可しないときはプレビューで知らせる（追記 B）（web の `huddle/call.test.ts`、実物）
- [x] ハドルの画面は別のタブに出る。そのタブを閉じても抜けず、チャットのタブの下にハドルの帯が出て、そこから開き直せる。タブを開けないときとモバイルでは同じタブに全画面で出る（追記 C）（web の `huddle/surface.test.ts`・`huddle-call.test.tsx`、実物）
- [x] Cloudflare の設定がなくてもサーバーは起動し、ハドルだけが無効になる（`TestLoadRealtime`・`TestHuddlesUnavailable`・`TestHuddleAPIUnavailable`）
- [x] ログに SDP・TURN の認証情報・アプリの秘密が出ない（実物の確認のあとの server のログを検査した。ハドルの要求 45 件で、どれも 0 件。2026-09-26）

### Phase 6.18c — ハドルの一覧・ハドル中の印・ハドルへのリンク

6.18a の後、6.18b より先に作る（オーナーの判断。2026-09-26）。どれも 6.18a の参加の状態・ハドルのメッセージ・`huddle_participants` の上に乗るので、芯が動いてから足す。
Slack の実物の画面（オーナーのスクリーンショット）で見つけたもの。ADR はフェーズの最初に書く。

- **サイドバーの「ハドルミーティング」の一覧**: 自分が入れる進行中のハドルと、自分が参加した過去のハドル（所要時間・参加者・ハドルのチャットへの入口）。
  Slack の一覧の画面（オーナーのスクリーンショット、2026-09-26）:
  - サイドバーの「スレッド」の下に「ハドルミーティング」の行。押すとメインの領域に一覧の画面
  - 右上に「＋ 新規ハドルミーティング」
  - 上に、よく使う場所のハドルを始める提案のカード（「#… でハドルミーティングを開始しますか？ 過去 1 週間にここで 1 回ハドルミーティングを実施しました」と相手の顔）
  - 「最近のハドルミーティング」: 絞り込み（すべてのハドルミーティング / 相手 / 場所）と、1 行ずつ「ヘッドフォンのアイコン・#ルーム名・7 時間前 · 2 分 · 1 件の返信（押すとスレッド）・参加者の顔・「…」のメニュー」
- **ハドル中の印**: 名前の横・メンバーの一覧・プロフィール・自分のアバターに、ハドル中であることだけを出す（どのルームかは出さない）。presence と同じく Redis に置き、WebSocket で配って REST で読み直す（ルール 5）
- **ハドルへのリンクのカード**: ハドルの画面の「リンクをコピー」と、本文に貼られたリンクのカード（進行中・参加中・終了）。見る人ごとに authz を通す（ADR 0040 と同じ）。入れるのはルームに投稿できる人だけのまま（ADR 0066 決定 7）

### Phase 6.18b — カメラと画面共有

6.18a の後に、別の ADR で決める（受けるトラックの選び方・simulcast・同時に映せる人数・拡大したハドルの窓・費用）。
