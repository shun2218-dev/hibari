...
6. **管理画面**: メンバー、招待、設定（3 つの PR に分ける。オーナーと確認）
   ...
   - 6-b 招待リンクの受け入れ（`/j/{code}`）
   - 6-c ユーザー設定（プロフィール・アバターの変更・デバイス・外観。表示の密度はデザインがないので出さない）
   - チャンネルの設定と DM の作成も、この構築順に含める（デザインに入口がないので、足されてから行う）
...
- [ ] 切断中に送信 → `failed` 表示 → 再送しても二重投稿されない
- [ ] 送信中にリロードしても二重投稿されない
- [ ] キックされたときに UI が即座に反映される
...
**フェーズの最初に決める**（実装の前に ADR にする）
- スキーマ: スレッドの親（`thread_root_id` など）、スレッドごとの返信数・最終返信時刻、スレッドへの参加（フォロー）と既読位置。`reply_to_id` を残すか消すか
- seq: スレッドの返信もルームの seq を使うか（チャンネルの並びに欠番が見える）、スレッドの中の並びをどう決めるか。change_seq による同期（ADR 0014）との関係
- 未読: チャンネルの未読を O(1) のまま求める方法（`last_message_seq` をチャンネルの投稿だけで進めるか、別の番号を持つか）。スレッドの未読の数え方と、一覧に出す場所
- 配信: スレッドの返信のイベント、親メッセージの返信数の更新、誰に届けるか（ルームの購読者か、スレッドの参加者か）
- 権限: スレッドへの投稿・編集・削除は、ルームの規則（ADR 0012）と同じでよいか。親が削除されたときのスレッド
- 画面: スレッドのパネル、親メッセージの「N 件の返信」、参加しているスレッドの一覧。`docs/ui/` にないので、先にデザインを足す

**DoD**（フェーズの最初に見直す）
- [ ] メッセージからスレッドを開いて返信でき、チャンネルのタイムラインには出ない
- [ ] 親メッセージに返信数と最終返信が出て、リアルタイムに更新される
- [ ] スレッドの返信でチャンネルの未読数が増えず、参加しているスレッドの未読が分かる
- [ ] 切断中に届いたスレッドの返信も、再接続の同期で揃う
