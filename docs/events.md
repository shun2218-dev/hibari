...
| `huddle_heartbeat` | `huddle_id`、`participant_id` | ハドルの心拍（ADR 0066 決定 5）。入っている間 **10 秒ごと**に送る。30 秒届かなければ外れる。参加がもうなければ `not_found`（外れた・別の端末に移った）で、クライアントは接続を片付ける |
| `activity` | `active` | この接続が画面を見ているか（ADR 0049）。**変わったときだけ**送る。接続は「見ていない」から始まるので、つないだ直後に 1 回送る。購読しているルームは関係ない（`room_id` / `workspace_id` を付けると `invalid_message`） |
